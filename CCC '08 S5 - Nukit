...
vector<vector<vector<vector<int>>>> states(31, vector<vector<vector<int>>>(31, vector<vector<int>>(31, vector<int>(31, -1)))); // 4D array to represent all possible states

bool solve(int a, int b, int c, int d) {
    if (a == 0 || b == 0 || c == 0 || d == 0) {
        return true; // if the reaction tried isn't possible, we can win from that reaction
    }
    ...
    // if we can force a losing position from this position (if we can't, then we're in a losing position)
